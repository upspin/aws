...
func main() {
	...
	if err := s.setInlineRolePolicy(role, bucketName); err != nil {
		s.Exitf("unable to set inline role policy: %s", err)
	}
...
func (s *state) setInlineRolePolicy(role *iam.Role, bucketName string) error {
	svc := iam.New(s.sess)
